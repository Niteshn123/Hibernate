...
public class TestCriteriaMain {
	...
	private static void testGroupBy() {
		...
		List li=s.createCriteria(CriteriaPOJO.class)
...
Iterator it = li.iterator();
	...
	private static void testOR1() {
		...
		c.add(Restrictions.or(
				Restrictions.eq("empSalary",2000000),
				Restrictions.or(
				Restrictions.like("empName", "n%"),
	...
	private static void testOR() {
		...
		Session s = sf.openSession();
		Criteria c= s.createCriteria(CriteriaPOJO.class);
		c.add(Restrictions.or(Restrictions.like("empName", "n%"), 
				Restrictions.eq("id",2)));
